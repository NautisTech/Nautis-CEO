...
const LoginFornecedor = ({ mode, dictionary }: { mode: SystemMode; dictionary: Awaited<ReturnType<typeof getDictionary>> }) => {
  // States
  ...
  const onSubmit: SubmitHandler<FormData> = async (data: FormData) => {
    ...
    try {
      const result = await signIn('credentials', {
        ...
        tenant_slug: tenantName.toLowerCase().trim(),
        redirect: false
      })
      ...
      if (result?.error) {
        ...
        // Login bem-sucedido - redirecionar para o portal de fornecedor
        toastService.success(`${dictionary['login']?.loginSuccess}`)

        // Aguarda um pouco para o toast aparecer antes do redirect
        setTimeout(() => {
          router.push(getLocalizedUrl('/fornecedor/dashboard', locale as Locale))
  ...
  const tenantDisplayName = process.env.NEXT_PUBLIC_TENANT_NAME || 'Nautis'
      ...
      <div className='flex justify-center items-center bs-full bg-backgroundPaper !min-is-full p-6 md:!min-is-[unset] md:p-12 md:is-[480px]'>
        <div className='flex flex-col gap-6 is-full sm:is-auto md:is-full sm:max-is-[400px] md:max-is-[unset] mbs-8 sm:mbs-11 md:mbs-0'>
          <div className='flex flex-col gap-1'>
            <Typography variant='h4'>{dictionary['login']?.welcomeTo?.replace("{{tenant}}", tenantDisplayName)}</Typography>
            ...
            <Controller
              ...
              render={({ field }) => (
                <CustomTextField
                  ...
                  autoFocus
                  fullWidth
                  label={dictionary['login']?.supplierCode}
