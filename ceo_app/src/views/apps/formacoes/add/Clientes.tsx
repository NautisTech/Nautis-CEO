...
const Clientes = ({ formacaoId }: ClientesProps) => {
  ...
  const [clientesAssociados, setClientesAssociados] = useState<number[]>([])
  ...
  const loadClientesAssociados = async () => {
    try {
      ...
      setClientesAssociados(data.map(c => c.id))
    } catch (error) {
  ...
  const handleOpenDialog = () => {
    setDialogOpen(true)
    setSelectedClientes([...clientesAssociados])
    loadClientes()
  }
  ...
  const handleSave = async () => {
    try {
      ...
      const clientesAdicionar = selectedClientes.filter(id => !clientesAssociados.includes(id))

      // Clientes a remover
      const clientesRemover = clientesAssociados.filter(id => !selectedClientes.includes(id))
      ...
      setClientesAssociados(selectedClientes)
      ...
      <CardContent>
        ...
        {clientesAssociados.length === 0 ? (
          ...
          <Typography variant='body2' color='text.secondary'>
            {clientesAssociados.length} cliente(s) associado(s) a esta formação
          </Typography>
        )}
      </CardContent>
