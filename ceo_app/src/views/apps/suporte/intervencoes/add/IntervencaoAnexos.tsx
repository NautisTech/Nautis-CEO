...
const IntervencaoAnexos = ({ viewOnly = false, intervencaoData }: Props) => {
  ...
  const maxAnexos = 10 // Limite para intervenções

  const { getRootProps, getInputProps } = useDropzone({
    onDrop: async (acceptedFiles: File[]) => {
      if (anexos.length + acceptedFiles.length > maxAnexos) {
        toastService.warning(`Máximo de ${maxAnexos} anexos permitidos`)
        return
      }
      ...
      try {
        ...
        setValue('anexos_ids', anexosIds)
      } catch (error) {
        toastService.error('Erro ao fazer upload dos anexos')
      } finally {
        setUploading(false)
      }
    },
  ...
  useEffect(() => {
    if (intervencaoData?.anexos) {
      const anexosExistentes: AnexoLocal[] = intervencaoData.anexos.map((a: any) => ({
        id: a.anexo_id || a.id,
        intervencao_anexo_id: a.id,
        nome: a.nome,
        ...
        tamanho_bytes: a.tamanho_bytes || a.tamanho || 0,
        uploaded: true,
  ...
  }, [intervencaoData, setValue, apiBase])
  ...
  const handleAddExternalUrl = async () => {
    ...
    try {
      ...
      const anexosIds = [...anexos, novoAnexo].filter(a => a.id).map(a => a.id!)
      setValue('anexos_ids', anexosIds)

      setUrlDialogOpen(false)
      setExternalUrl('')
      setUrlError('')
      toastService.success('URL externa adicionada com sucesso')
    } catch (error) {
      setUrlError('Erro ao registar URL externa')
    }
  }
  ...
  const handleRemove = async (index: number) => {
    ...
    if (anexo.id) {
      try {
        ...
        toastService.error('Erro ao remover anexo')
        return
      }
    }
    ...
    toastService.success('Anexo removido com sucesso')
  }
  ...
  const getFileIcon = (anexo: AnexoLocal) => {
    ...
    if (tipo.includes('doc') || mimeType.includes('word')) {
      return 'tabler-file-type-doc'
    }
    if (tipo.includes('xls') || tipo.includes('sheet') || mimeType.includes('excel') || mimeType.includes('spreadsheet')) {
    ...
    <>
      <Dropzone>
        <Card>
          <CardHeader
            title='Anexos'
            subheader={`Adicione documentos, imagens ou ficheiros à intervenção (máximo ${maxAnexos})`}
            action={
              <Chip
          ...
          <CardContent className='space-y-4'>
            {anexos.length < maxAnexos && !viewOnly && (
              <div {...getRootProps({ className: 'dropzone' })}>
                ...
                <div className='flex items-center flex-col gap-2 text-center'>
                  ...
                  <Typography variant='body1'>Arraste ficheiros para aqui</Typography>
            ...
            {uploading && (
              <Alert severity='info' icon={false}>
                ...
                <Typography variant='body2'>A fazer upload de ficheiros...</Typography>
              </Alert>
            )}
                      ...
                      <div className='flex-1 min-w-0'>
                        ...
                        <div className='flex items-center gap-2 flex-wrap'>
                          ...
                          {anexo.tipo_documento && (
                            <Chip label={anexo.tipo_documento} size='small' color='primary' variant='tonal' />
                          )}
                          {anexo.variants && (
                            <Chip label='Otimizado' size='small' color='success' variant='tonal' />
                          )}
                        </div>
                        {anexo.descricao && (
                          <Typography variant='caption' color='text.disabled'>
      ...
      >
        ...
        <DialogContent>
          <CustomTextField
            ...
            error={!!urlError}
            helperText={urlError || 'Cole o link direto para o ficheiro (imagem, vídeo, documento, etc.)'}
            placeholder='https://example.com/documento.pdf'
          />
          <Alert severity='info' className='mt-4'>
            Ficheiros externos não serão otimizados automaticamente pelo sistema.
          </Alert>
        </DialogContent>
        <DialogActions>
          <Button
...
export default IntervencaoAnexos
